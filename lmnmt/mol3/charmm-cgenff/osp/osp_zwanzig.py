...
#-----------------------------------------------------------------
#------------------------------------------------------------------
print(f'''In the free energy cycle we need;
cost of adding restraints on xray pose
cost of removinig restraints from flipped pose
''')

win = 1
out = ['forxray', 'forflip']

for pose in range(len(out)):
    forw = []
    back = []
    dGs  = []
    for myrep in range(3):
        f1 = np.genfromtxt(f'win{win}/{out[pose]}/win{win-1}_{myrep}.dat', dtype=None, skip_header=1, usecols=-1, delimiter=' ')
        f0 = np.genfromtxt(f'win{win}/{out[pose]}/win{win}_{myrep}.dat', dtype=None, skip_header=1, usecols=-1, delimiter=' ')
        f2 = np.genfromtxt(f'win{win}/{out[pose]}/win{win+1}_{myrep}.dat', dtype=None, skip_header=1, usecols=-1, delimiter=' ')
    ...
    print(f'Free energy of removing the restraints from {out[pose][3:]} pose:')
    print(f'{round(final_dG, 1)} +/- {round(std, 1)}')
